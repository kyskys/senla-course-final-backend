...
public class PairDaoImpl extends SearchableDaoImpl<PairSearchParams, Pair> implements PairDao {
	...
	/*
	 * @Override public void addMarkToPair(Long idMark, Long idPair) { Session
	 * session = getSession(); Query query =
	 * session.createQuery("update Mark set pair.id = :idPair where id= :idMark"
	 * ); query.setParameter("idMark", idMark); query.setParameter("idPair",
	 * idPair); query.executeUpdate(); }
	 */
